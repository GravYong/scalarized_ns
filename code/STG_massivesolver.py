...
if switch:
 if 1:
   print( zz.TOV_solver(639024424554650.0, 0.11441417623153093, verbose=1) ) 
 else:
   zzz = zz.TOV_iterator( 4e14 )
   print(zzz['m_A']/MSUN, zzz )
...
if not switch:
 ...
 highec = 12e14
 ecm = highec
 mtest = zz.TOV_iterator( ecm )['m_A']
 ic = 0
 while abs( (mtest - admm)/admm) > 1.0e-4:
   ecm = np.sqrt(lowec*highec)
   res = zz.TOV_iterator( ecm )
   mtest = res['m_A']
   #print(mtest/MSUN - 1.4)
   if mtest > admm:
      highec = ecm
   else:
      lowec = ecm
   ic +=1
 phc = res['varphi_c']
 print( zz.TOV_solver(ecm, phc, verbose=1) ) 
